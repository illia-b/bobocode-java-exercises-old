...
public class CrazyStreams {
    ...
    private static <T, V> Predicate<T> having(Function<T, V> by, V asValue) {
        return t -> Predicate.isEqual(asValue).test(by.apply(t));
    }
    ...
    public List<Account> findAccountsByBirthdayMonth(Month birthdayMonth) {
        return accounts.stream().filter(account ->
            Predicate.isEqual(birthdayMonth).test(account.getBirthday().getMonth())
        ).toList();
    }
    ...
    public Map<Boolean, List<Account>> partitionMaleAccounts() {
        return accounts.stream().collect(Collectors.partitioningBy(
                having(Account::getSex, Sex.MALE)));
    }
    ...
    public BigDecimal calculateTotalBalance() {
        ...
        BigDecimal add = one.add(BigDecimal.TEN);
    ...
    public Map<String, BigDecimal> collectBalancesByEmailForAccountsCreatedOn(int year) {
        return accounts.stream()
                .filter(account ->
                        account.getCreationDate().getYear() == year)
    ...
    public Map<Character, Long> getCharacterFrequencyInFirstNames() {
        return accounts.stream().flatMap(account -> account.getFirstName().chars().mapToObj(i -> (char) i))
    ...
    public Map<Character, Long> getCharacterFrequencyIgnoreCaseInFirstAndLastNames() {
        return accounts.stream()
                ...
                .flatMap(names -> names.chars().mapToObj(i -> (char) i))
                .collect(Collectors.groupingBy(Function.identity(), Collectors.counting()));
    }
